...
FROM dpd.drug_product as drug_new LEFT JOIN dpd_current.drug_product as drug_old ON
...
inner join dpd_current.ingredient_stem_csv as isc
...
LEFT JOIN dpd_current.drug_product as drug_old ON drug_old.drug_code = dpd_changes.drugs.drug_code
...
LEFT JOIN dpd_current.companies as drug_old ON drug_old.drug_code = dpd_changes.drugs.drug_code
...
FROM dpd.status as drug_new LEFT JOIN dpd_current.status as drug_old ON
...
WHERE (
  ...
  drug_new.drug_code NOT IN (
    ...
    LEFT JOIN dpd_current.status as drug_old ON drug_new.drug_code=drug_old.drug_code AND drug_new.current_status_flag = 'Y' AND drug_old.current_status_flag = 'Y'
    WHERE
...
FROM dpd_current.route AS drug_old
...
FROM dpd_current.pharmaceutical_form AS drug_old
...
FROM (
  SELECT s.drug_code, s.active_ingredient_code, s.dosage_value, s.strength, min(s.status::text) FROM (
    ...
    FROM dpd_current.active_ingredient AS ingredient_old))
  ) s
...
FROM dpd_current.active_ingredient AS drug_old) ) s
WHERE drug_code IN (SELECT drug_code from dpd_changes.drugs)
GROUP BY (drug_code, active_ingredient_code)
HAVING count(*) = 1
);
