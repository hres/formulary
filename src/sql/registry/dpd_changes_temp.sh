...
# set database
export PGDATABASE="registry_temp"
baseDir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

# rename existing schema #1
psql -d registry_temp -c "ALTER SCHEMA $SRC_SCHEMA RENAME TO $SRC_SCHEMA_TEMP"

# create new dpd records
./dpd_import.sh

# rename existing schema #2
psql -d registry_temp -c "ALTER SCHEMA $SRC_SCHEMA RENAME TO $COMPARISON_SCHEMA"

# reverse renaming #1
psql -d registry_temp -c "ALTER SCHEMA $SRC_SCHEMA_TEMP RENAME TO $SRC_SCHEMA"

# create and populate ccdd.ingredient_stem for comparison
psql -d registry_temp -c "CREATE TABLE $COMPARISON_SCHEMA.ingredient_stem_csv(
	ccdd varchar,
	top250name varchar,
...
psql -d registry_temp -c "DROP TABLE if exists public.ccdd_config CASCADE"
pgloader "$baseDir/../ccdd-config.pgload"

# drop dpd_changes schema if exists 
psql -d registry_temp -c "DROP SCHEMA IF EXISTS $CHANGE_SCHEMA CASCADE"

# process changes
psql -d registry_temp < ./changes.sql
...
psql -d registry_temp -c "DROP SCHEMA $COMPARISON_SCHEMA CASCADE"