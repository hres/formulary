...
-- object: ccdd.special_groupings | type: MATERIALIZED VIEW --
-- DROP MATERIALIZED VIEW IF EXISTS ccdd.special_groupings CASCADE;
...
from
	...
	LEFT JOIN (
	  SELECT mp_code, mp_status, mp_formal_name, mp_fr_description, mp_status_effective_time, mp_type, "Health_Canada_identifier", "Health_Canada_product_name" FROM ccdd_mp_table
	  UNION
	  SELECT mp_code, mp_status, mp_formal_name, mp_fr_description, mp_status_effective_time, mp_type, "Health_Canada_identifier", "Health_Canada_product_name" FROM ccdd_mp_carry_forward
	) nxt ON(nxt.mp_code = cur.mp_code)
	LEFT JOIN (SELECT drug_code, drug_identification_number FROM dpd.drug_product) din_and_drug_code
    ON (din_and_drug_code.drug_identification_number = cur.mp_code)
	LEFT JOIN LATERAL (VALUES
		('mp_formal_name', cur.mp_formal_name, nxt.mp_formal_name),
...
-- object: public.release_changes_special_groupings | type: VIEW --
-- DROP VIEW IF EXISTS public.release_changes_special_groupings CASCADE;
