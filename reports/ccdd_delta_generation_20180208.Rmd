...
The Combination products file has changes to the NTP_TYPE column only:
...
dpd_new_product_existing_tm <- dpd_human_ccdd_products %>%
                                ...
                                left_join(ing_stem_feb %>% select(ingredient = dpd_ingredient, ccdd), copy = TRUE) %>%
...
max_tm_code <- max(as.integer(tm_full_release_20180123$tm_code), na.rm = TRUE)

new_tms <- ccdd_mp_source %>% 
           filter(is.na(tm_code), ccdd == TRUE) %>%
...
new_ntps <- ccdd_mp_source %>%
            filter(is.na(ntp_code), ccdd == TRUE) %>%
...
ccdd_mp_table_additions <- ccdd_mp_table %>%
                            ...
                            select(mp_code, drug_identification_number, brand_name, reason) %>%
...
ccdd_ntp_table_additions <- ccdd_ntp_table %>%
                            mutate_all(as.character) %>%
                            anti_join(ntp_full_release_20180123 %>% as.tibble() %>% mutate_all(as.character))

ccdd_tm_table_additions <- ccdd_tm_table %>% 
                           mutate_all(as.character) %>%
                           anti_join(tm_full_release_20180123 %>% as.tibble() %>% mutate_all(as.character))

mp_ntp_tm_relationship_delta <- filter(ccdd_mp_source, tm_code %in% ccdd_tm_table_additions$tm_code) %>% select(tm_code, tm_formal_name, ntp_code, ntp_formal_name, mp_code, mp_formal_name, Health_Canada_identifier = drug_identification_number, Health_Canada_description = descriptor) %>% arrange(tm_code, ntp_code, mp_code)
