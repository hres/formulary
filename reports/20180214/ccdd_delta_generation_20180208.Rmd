...
dpd_new_product_existing_tm <- dpd_human_ccdd_products %>%
                                ...
                                semi_join(ing_stem_feb %>% select(ingredient = dpd_ingredient, ccdd), copy = TRUE) %>%
                                anti_join(mp_to_din_or_npn_mapping_20180123 %>% select(drug_identification_number = Health_Canada_identifier), copy = TRUE) %>%
                                collect() %>%
...
dpd_delta <- bind_rows(ccdd_added_tm, ccdd_uop_changed, dpd_changed, dpd_new_product_existing_tm)
 ...
 # left_join(tm_full_release_20180123 %>% select(tm_formal_name, tm_code), copy = TRUE) %>%
  ...
  mutate(mp_formal_name = if_else(is.na(combo_mp_formal_name),
         ...
         mp_status = case_when(current_status == "MARKETED" ~ "active",
                               ...
                               TRUE ~ "inactive")) %>%
  left_join(ntp_full_release_20180123) %T>%
...
max_ntp_code <- max(as.integer(ntp_full_release_20180123$ntp_code))
...
ccdd_mp_table_additions <- ccdd_mp_table %>%
                            ...
                            anti_join(mp_full_release_20180123 %>% as.tibble() %>% mutate_all(as.character)) %>%
...
ccdd_mp_table_changes <- ccdd_mp_table %>%
                          ...
                          mutate_at("mp_status_effective_time", format, "%Y%m%d") %>%
                          select(Health_Canada_identifier = drug_identification_number, everything())

ccdd_ntp_table_additions <- ccdd_ntp_table %>%
                            mutate_at("ntp_code", as.character) %>%
                            anti_join(ntp_full_release_20180123 %>% as.tibble(), by = c("ntp_code")) %>%
...
ccdd_ntp_table_changes <- ccdd_ntp_table %>%
                            ...
                            mutate_at("ntp_code", as.character) %>%
                            mutate_at("ntp_status_effective_time", format, "%Y%m%d") %>%
                            anti_join(ntp_full_release_20180123 %>% as.tibble())

ccdd_tm_table_additions <- ccdd_tm_table %>% 
                           as.tibble() %>%
                           select(tm_code, tm_formal_name) %>%
                           mutate_all(as.character) %>%
                           anti_join(tm_full_release_20180123 %>% as.tibble())
...
datatable(ccdd_mp_table_additions)
datatable(ccdd_mp_table_changes)
datatable(ccdd_ntp_table_additions)
datatable(ccdd_tm_table_additions)
datatable(mp_ntp_tm_relationship_delta)
```

Apply changes to previous month's dataset
```{r ccdd_update, echo = FALSE, message = FALSE}
mp_full_release_20180214 <- mp_full_release_20180123 %>% 
                            left_join(mp_to_din_or_npn_mapping_20180123 %>% select(mp_code, Health_Canada_identifier)) %>%
                              as.tibble() %>%
...
ntp_full_release_20180214 <- ntp_full_release_20180123 %>%
                              as.tibble() %>%
...
tm_full_release_20180214 <- tm_full_release_20180123 %>%
                              as.tibble() %>%
...
mp_ntp_tm_relationship_20180214 <- mp_ntp_tm_relationship_20180123 %>%
                                    as.tibble() %>%
                                    anti_join(mp_ntp_tm_relationship_delta, by = c("tm_code", "ntp_code", "mp_code")) %>%
                                   bind_rows(mp_ntp_tm_relationship_delta)

write.csv(mp_full_release_20180214, file = "mp_full_release_20180214.csv", row.names = FALSE)
write.csv(ntp_full_release_20180214, file = "ntp_full_release_20180214.csv", row.names = FALSE)
write.csv(tm_full_release_20180214, file = "tm_full_release_20180214.csv", row.names = FALSE)
write.csv(mp_ntp_tm_relationship_20180214, file = "mp_ntp_tm_relationship_20180214.csv", row.names = FALSE)

datatable(mp_full_release_20180214)
datatable(ntp_full_release_20180214)
datatable(tm_full_release_20180214)
datatable(mp_ntp_tm_relationship_20180214)
```
