...
#cvzipurl <-"http://www.hc-sc.gc.ca/dhp-mps/alt_formats/zip/medeff/databasdon/cvponline_extract_20160331.zip"
...
library(dplyr)
hcopen <- src_postgres(dbname = "hcopen", host = "shiny.hc.local", user = "hcwriter", password = "canada2")
for(i in ls(pattern = "cv_")){
  copy_to(hcopen, as.data.frame(get(i)), name = paste0(i, "_20161231"), temporary = FALSE, indexes = as.list(names(get(i))))
}

copy_to(hcopen, as.data.frame(cv_reactions), name = "cv_reactions_20161231", temporary = FALSE, indexes = as.list(names(cv_reactions)))
copy_to(hcopen, as.data.frame(cv_report_drug), name = "cv_report_drug_20161231", temporary = FALSE, indexes = as.list(names(cv_report_drug)))
copy_to(hcopen, as.data.frame(cv_report_drug_indication), name = "cv_report_drug_indication_20161231", temporary = FALSE, indexes = as.list(names(cv_report_drug_indication)))
copy_to(hcopen, as.data.frame(cv_report_links), name = "cv_report_links_20161231", temporary = FALSE, indexes = as.list(names(cv_report_links)))
copy_to(hcopen, as.data.frame(cv_report_type), name = "cv_report_type_20161231", temporary = FALSE, indexes = as.list(names(cv_report_type)))
copy_to(hcopen, as.data.frame(cv_seriousness), name = "cv_seriousness_20161231", temporary = FALSE, indexes = as.list(names(cv_seriousness)))
copy_to(hcopen, as.data.frame(cv_source), name = "cv_source_20161231", temporary = FALSE, indexes = as.list(names(cv_source)))
copy_to(hcopen, as.data.frame(cv_reports), name = "cv_reports_20161231", temporary = FALSE, indexes = as.list(names(cv_reports)))
